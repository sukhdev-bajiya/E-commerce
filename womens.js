...
for (var i = 0; i < womensData.length; i++) {
  womensData[i].id = 100 + i;
  womensData[i].quantiItem = 1;
}

womensData.map(function (element) {
  var off = Math.round(
    ((element.strikedoffprice - element.price) / element.strikedoffprice) * 100
  );
  var item = `
              ...
              <div>
                  <div>
                      ...
                      <p>${element.strikedoffprice}</p>
                  </div>
                  <span>${off}% off</span>
              </div>
              <button type="submit" onclick="addProductToCart(${element.id})">Add to Cart</button>
          `;
...
var myProduct = JSON.parse(localStorage.getItem("addToCart")) || [];

function addProductToCart(val) {
  var flag = true;
  for (var i = 0; i < myProduct.length; i++) {
    if (myProduct[i].id == val) {
      flag = false;
      break;
    }
  }
  if (flag) {
    for (var i = 0; i < womensData.length; i++) {
      if (womensData[i].id == val) {
        myProduct.push(womensData[i]);
      }
    }
    localStorage.setItem("addToCart", JSON.stringify(myProduct));
  }
  var cartTotalPrice = document.getElementById("cartCount");
  cartTotalPrice.innerText = myProduct.length;
}
var cartTotalPrice = document.getElementById("cartCount");
  cartTotalPrice.innerText = myProduct.length;